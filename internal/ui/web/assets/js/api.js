...
const API = (() => {
  ...
  return {
    ...
    dnsDetected: () => req('GET', '/api/dns-providers/detected'),
    importDNS: (code) => req('POST', '/api/dns-providers/import', { code }),
    deleteCert: (id, purge) => req('DELETE', '/api/certs/' + encodeURIComponent(id) + (purge ? '?purge=1' : '')),
    tlsCheck: (id, host) => req('GET', '/api/certs/' + encodeURIComponent(id) + '/tls-check' + (host ? '?host=' + encodeURIComponent(host) : '')),
    certPem: (id, file) => req('GET', '/api/certs/' + encodeURIComponent(id) + '/pem?file=' + encodeURIComponent(file)),
    certDownloadUrl: (id, file, confirm) => '/api/certs/' + encodeURIComponent(id) + '/download?file=' + encodeURIComponent(file) + (confirm ? '&confirm=1' : ''),
    setDefaultCA: (ca) => req('POST', '/api/acme/default-ca', { ca }),
